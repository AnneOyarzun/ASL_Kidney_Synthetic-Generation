...
class CycleGANModel(BaseModel):
    ...
    def backward_G(self):
        ...
        if self.opt.lambda_gradient:
            lambda_gradient = self.opt.lambda_gradient  # New lambda parameter for gradient difference loss

        # Loss function → perceptual loss function (link: https://github.com/EliasVansteenkiste/CycleGANwithPerceptionLoss/commit/fb082e556c4a1af8cc8505195c1dcb41af029e46)
        if self.opt.lambda_feat_AfB:
        ...
        if self.opt.lambda_gradient:
        # Calculate generator gradient loss using custom gradient function
            gradient_loss_A2B = lambda_gradient * (
                torch.nn.MSELoss()(gradient(self.fake_B, 0), gradient(self.real_A, 0)) +
                torch.nn.MSELoss()(gradient(self.fake_B, 1), gradient(self.real_A, 1))
            )
            gradient_loss_B2A = lambda_gradient * (
                torch.nn.MSELoss()(gradient(self.fake_A, 0), gradient(self.real_B, 0)) +
                torch.nn.MSELoss()(gradient(self.fake_A, 1), gradient(self.real_B, 1))
            )
        
            # Scale SSIM loss by lambda parameter
            self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + gradient_loss_A2B + gradient_loss_B2A 
        
        elif self.opt.lambda_feat_AfB:
