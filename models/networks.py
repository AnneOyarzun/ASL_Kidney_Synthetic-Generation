...
# For perceptual loss function (link:https://github.com/EliasVansteenkiste/CycleGANwithPerceptionLoss/blob/master/models/networks.py#L454)
...
#         state_dict = torch.utils.model_zoo.load_url(ResNet34_Weights.IMAGENET1K_V1.url)
...
# lo he modificado yo (necesito channel 1 inputs)
class FeatureResNet18(nn.Module):
    def __init__(self, gpu_ids, **kwargs):
        ...
        # The original ResNet18 model expects a 3-channel input
        # We need to adjust the number of input channels to match our single-channel input
        # The new convolutional layer will have 1 input channel instead of 3
...
def define_feature_network(which_model_netFeat, gpu_ids=[]):
    ...
    elif which_model_netFeat == 'resnet18':
        netFeat = FeatureResNet18(gpu_ids=gpu_ids)
    else:
